...
public class WakeControllerPeripheralProvider implements IPeripheralProvider {

    @NotNull
    @Override
    public LazyOptional<IPeripheral> getPeripheral(@NotNull Level level, @NotNull BlockPos pos, @NotNull Direction side) {
        BlockEntity be = level.getBlockEntity(pos);
        if (be instanceof WakeControllerBlockEntity controller) {
            return LazyOptional.of(() -> new WakeControllerPeripheral(controller));
        }
